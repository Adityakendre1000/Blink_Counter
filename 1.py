...
while True:
    ...
    if faces:
        face = faces[0]
        for id in idlist:
            # Defining parameters for visualizing facial landmarks
            radius = 3  # Reducing the radius of the circle to make it smaller
            thickness = -1  # Filling the circle rather than drawing the outline

            # Drawing circles at specific facial landmarks
            cv2.circle(img, face[id], radius, (255, 0, 255), thickness)
